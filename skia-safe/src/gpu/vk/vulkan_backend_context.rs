...
pub use super::vulkan_backend_context_builder::BackendContextBuilder as Builder;

// The `GrBackendContext` memory layout generated by bindgen does not match in size, so we do need
// to use a pointer here for now.
...
impl BackendContext<'_> {
    ...
    pub(crate) unsafe fn new_internal<'a>(
        instance: Instance,
        physical_device: PhysicalDevice,
        device: Device,
        (queue, queue_index): (Queue, usize),
        get_proc: &'a dyn GetProc,
        max_api_version: Option<Version>,
        protected_context: gpu::Protected,
        instance_extensions: &[&str],
        device_extensions: &[&str],
    ) -> BackendContext<'a> {
