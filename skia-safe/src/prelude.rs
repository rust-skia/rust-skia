...
pub struct Handle<N: NativeDrop>(
    // UnsafeCell to disable niche optimization (we often don't care about the private types in
    // bindgen).
    UnsafeCell<N>,
    // `*const` is needed to prevent automatic Send and Sync trait implementation, which happens
    // when the underlying type generated by bindgen is Send and Sync.
    PhantomData<*const ()>,
);
...
impl<N: NativeDrop> Handle<N> {
    ...
    pub(crate) fn replace_native(mut self, native: &mut N) -> Self {
        mem::swap(self.0.get_mut(), native);
        self
    }
...
impl<N: NativeDrop + NativeClone> Clone for Handle<N> {
    fn clone(&self) -> Self {
        Self::from_native_c(self.native().clone())
    }
}
